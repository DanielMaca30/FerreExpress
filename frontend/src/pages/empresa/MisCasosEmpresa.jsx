...
// Si en tu backend tienes endpoints namespaced para empresas, por ejemplo
// "/empresa/casos", cambia estas constantes aquí y listo.
...
export default function MisCasosEmpresa() {
  ...
  /* ==== cargar casos (empresa/contratista autenticado) ==== */

  const fetchCasos = async ({ silent = false } = {}) => {
    if (!silent) setLoading(true);
    try {
      // El backend usa el usuario (rol CONTRATISTA/EMPRESA) del JWT
      ...
      toast({
        ...
        description:
          "No fue posible cargar los casos de soporte de tu empresa.",
      });
    } finally {
  ...
  const handleAddComentario = async () => {
    ...
    try {
      ...
      const { data } = await api.post(`${CASOS_BASE}/${selected.id}/comentarios`, {
        mensaje: msg,
      });
  ...
  const handleCreateCaso = async () => {
    ...
    try {
      ...
      toast({
        ...
        title: "Caso creado",
        description:
          "Tu caso de soporte empresarial fue creado correctamente. Podrás seguir aquí las respuestas.",
      });
      setIsCreateOpen(false);
      resetCreateForm();
    } catch (err) {
  ...
  return (
    ...
    >
        ...
        <VStack align="flex-start" spacing={1}>
          <HStack spacing={2}>
            <Heading size="lg" color={title}>
              Soporte y casos de empresa
            </Heading>
            <Tag size="sm" variant="subtle" colorScheme="purple">
              <TagLeftIcon as={FiLifeBuoy} />
              <TagLabel>Solicitudes de tu empresa</TagLabel>
            </Tag>
          </HStack>
          <Text fontSize="sm" color={muted}>
            Aquí puedes crear y gestionar casos de soporte relacionados con
            tus cotizaciones, pedidos B2B, facturación y acceso para tu empresa.
          </Text>
        </VStack>

        <HStack spacing={2}>
          <Tooltip label="Recargar casos">
            <IconButton
      ...
      >
        ...
        {loading ? (
          ...
          <VStack py={6} spacing={2}>
            <FiInfo />
            <Text fontSize="sm" color={muted}>
              Aún no tienes casos de soporte registrados con los filtros
              actuales para tu empresa.
            </Text>
            <Button
              size="sm"
              leftIcon={<FiPlus />}
      ...
      <Modal isOpen={isCreateOpen} onClose={handleCloseCreate} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Nuevo caso de soporte para tu empresa</ModalHeader>
            ...
            <VStack align="stretch" spacing={3}>
              <Box>
                ...
                <Input
                  ...
                  placeholder="Ej: Problema con factura electrónica, error en cotización..."
                />
              </Box>
              <Box>
                <Text fontSize="sm" mb={1}>
              ...
              <Box>
                ...
                <Textarea
                  ...
                  placeholder="Cuéntanos qué pasó, qué esperabas y cualquier dato clave (ID de cotización, NIT, pedido, etc.)."
                />
              </Box>
            </VStack>
          </ModalBody>
                ...
                <Box>
                  ...
                  >
                    {loadingComentarios ? (
                      ...
                      <Text fontSize="xs" color={muted}>
                        Aún no hay mensajes en este caso. El equipo de
                        FerreExpress responderá aquí las solicitudes de tu
                        empresa.
                      </Text>
                    ) : (
                      <VStack align="stretch" spacing={2}>
                ...
                <Box>
                  ...
                  <VStack align="stretch" spacing={2}>
                    <Textarea
                      ...
                      placeholder="Actualiza el caso con más detalles, adjunta contexto de cotización/pedido o responde al equipo de soporte."
                    />
