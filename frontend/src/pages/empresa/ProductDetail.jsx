...
import {
  ...
  FiShoppingBag,
  FiCheck,
  FiFileText, // 🆕 para botón de cotización y ficha técnica
} from "react-icons/fi";
import { motion } from "framer-motion";
import api, { API_BASE_URL } from "../../utils/axiosInstance";
import { addToCart } from "../../utils/cartStore";
import { useAuth } from "../../context/AuthContext"; // 🆕 para saber rol
...
/* ====================== Borrador de Cotización (localStorage) ====================== */
/**
 * Guarda un borrador de cotización en localStorage.
 * Estructura:
 * {
 *   updatedAt: ISOString,
 *   items: [ { id, nombre, precio_unitario, qty } ]
 * }
 */
const addToQuoteDraft = (producto, qty) => {
  if (!producto) return;
  try {
    const raw = localStorage.getItem("cotizacionDraft") || "null";
    const current = JSON.parse(raw) || { items: [] };

    if (!Array.isArray(current.items)) current.items = [];

    const idx = current.items.findIndex(
      (it) => String(it.id) === String(producto.id)
    );
    const unit = unitPriceFrom(producto);
    if (idx >= 0) {
    ...
    localStorage.setItem("cotizacionDraft", JSON.stringify(current));
    return current;
  } catch {
    // si algo falla, simplemente no rompemos la UI
    return null;
  }
};

/* ================= Relacionados (Lógica intacta) ================= */
...
export default function ProductDetail() {
  ...
  const isEmpresa =
    user?.role === "CONTRATISTA" || user?.role === "EMPRESA"; // 🆕
  ...
  }, [producto, clampedQty, navigate]);

  /* 🆕 Agregar a cotización (solo empresas/contratistas) */
  const handleAddToQuote = useCallback(() => {
    if (!producto) return;
    const qtySafe = Math.max(1, clampedQty);
    const result = addToQuoteDraft(producto, qtySafe);
    toast({
      title: "Agregado a cotización",
      description: `${producto.nombre} • ${qtySafe} und. añadidas al borrador de cotización.`,
      status: "info",
      duration: 1800,
      position: "top-right",
      isClosable: true,
    ...
    if (result) {
      // Podrías, a futuro, redirigir a /empresa/cotizacion-preview aquí si quieres:
      // navigate("/empresa/cotizaciones/borrador");
    }
  }, [producto, clampedQty, toast]);
  ...
  if (loading && !producto) {
    const { cardBg, shadowLg } = useSurfaceTokens();
    return (
            ...
            >
              ...
              <HStack justify="space-between" align="start">
                <VStack align="start" spacing={1}>
                  ...
                  {/* 🆕 mini resumen para rol empresa */}
                  {isEmpresa && (
                    <Text fontSize="xs" color="gray.500" mt={2}>
                      Ideal para cotizar {clampedQty} und. — subtotal estimado{" "}
                      <strong>{fmtCop(totalPrice)}</strong>
                    </Text>
                  )}
                </VStack>
              ...
              <VStack mt={6} spacing={3} w="full">
                ...
                {/* 🆕 Botón para cotización B2B */}
                {isEmpresa && (
                  <Tooltip label="Añadir este producto a tu borrador de cotización para empresa">
                    <Button
                      w="full"
                      h="48px"
                      variant="ghost"
