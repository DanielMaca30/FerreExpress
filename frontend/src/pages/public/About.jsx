...
import { keyframes } from "@chakra-ui/system"; // ✅ compatible con tu versión
...
// Wrapper con animación de entrada (fade + slide)
...
export default function About() {
  // ---- estado de productos para el hero ----
    ...
    <Box bg={pageBg} px={{ base: 3, md: 6, lg: 10 }} py={{ base: 4, md: 6 }}>
      <MotionSection>
        <HeroAbout
          ...
          items={ofertas}
          onView={onView}
          onGoProposal={() => navigate("/register/empresa")}                         // ← /login
          onGoHow={() => scrollToId("como-funciona-por-rol", { pulse: true })} // ← scroll con relieve
        />
      </MotionSection>
        ...
        <Section title="Propuesta de valor" mt={5} id="propuesta-de-valor">
          <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
            <InfoCard icon={FiBox} title="Catálogo claro y búsqueda rápida">
              <Text>
                Fichas técnicas organizadas, filtros y resultados relevantes para decidir
                mejor.
              </Text>
            </InfoCard>
            <InfoCard icon={FiFileText} title="Proformas B2B con vigencia visible">
              <Text>
                Cotizaciones con descuentos por volumen y conversión a pedido validando
                stock.
              </Text>
            </InfoCard>
            <InfoCard icon={FiShoppingCart} title="Compra segura y sin fricción">
              <Text>
                Carrito persistente y flujo de pago listo para demo con pasarela simulada
                end-to-end.
              </Text>
            </InfoCard>
            <InfoCard icon={FiUsers} title="Soporte por rol con SLA">
              <Text>
                Centro de ayuda con FAQ por tipo de usuario y apertura de casos con estados
        ...
        <Section title="Lo que estamos construyendo" mt={5}>
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4}>
            <InfoCard icon={FiTruck} title="Logística y entregas flexibles">
              <Text>
                Integraciones de última milla y métodos de envío por zona y prioridad.
              </Text>
            </InfoCard>
            <InfoCard icon={FiActivity} title="Métricas operativas en tiempo real">
              <Text>
                Visibilidad de tiempos de preparación, pedidos activos y estado de casos.
              </Text>
            </InfoCard>
      ...
      <MotionSection>
        <Section title="Contexto del mercado" mt={5}>
          <InfoCard>
            <Text>
              La digitalización del comercio en Colombia mantiene un crecimiento sostenido y el
              canal online se consolida para consumo y abastecimiento empresarial. En ferretería y
              construcción, la migración a plataformas digitales agiliza compras recurrentes,
              cotizaciones y abastecimiento por proyecto, haciendo más eficiente la cadena desde el
              catálogo hasta la entrega.
            </Text>
          </InfoCard>
        </Section>
      </MotionSection>
        ...
        <Section mt={5}>
          <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
            <InfoCard title="Visión">
              <Text>
                Ser la plataforma de referencia en compras ferreteras en Colombia, uniendo
                catálogo, cotización B2B y gestión de pedidos en un flujo claro y confiable, con
                servicio humano y tecnología que realmente ahorra tiempo y costos.
              </Text>
            </InfoCard>
      ...
      >
        <VStack align="start" spacing={3} flex={1}>
          ...
          <Text color={fg}>
            ...
            Atendemos clientes finales y contratistas/empresas con catálogo claro, proformas
            con vigencia, descuentos por volumen y seguimiento de pedidos en tiempo real.
          </Text>
          <HStack>
            <Button
              rightIcon={<FiChevronsRight />}
              colorScheme="teal"
              onClick={onGoProposal}             // ← /login
            >
              Ver propuesta
            </Button>
        ...
        {/* Derecha: tarjetas reales en formato compacto con animación de aparición */}
        <Box flex={1} w="full">
          <Box
            borderRadius="xl"
            p={4}
...
function Section({ title, children, mt = 0, id }) {
  ...
  // Resalte breve (teal) al aterrizar
  ...
  return (
    <motion.div
      // micro-animación de hover (sutil) para todo el contenedor
    ...
    >
      <Box
        id={sectionId}
        bg={cardBg}
        // :target por si navegan con hash; además soportamos un pulso programático via data-pulse
        sx={{
...
/* ========== Subcomponentes de catálogo (compactos para Hero) ========== */
...
// Scroll con offset real del header + pulso garantizado
function scrollToId(id, { offset, pulse = false } = {}) {
  const el = document.getElementById(id);
  if (!el) return;

  const headerOffset = Number.isFinite(offset) ? offset : getStickyHeaderOffset();
  const y = window.pageYOffset + el.getBoundingClientRect().top - headerOffset - 8; // -8 por respiración visual

  window.scrollTo({ top: y, behavior: "smooth" });

  // Actualiza hash (por accesibilidad y navegación)
  if (history.pushState) history.pushState(null, "", `#${id}`);
  else window.location.hash = `#${id}`;

  // Accesibilidad: foco + relieve programático
...
// Detecta altura del header sticky/fixed (toma el primero que encuentre)
